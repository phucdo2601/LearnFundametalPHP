...
@section('content')
  <main class="pt-90">
    ...
    <div class="shop-sidebar side-sticky bg-body" id="shopFilter">
      ...
      <div class="accordion-item mb-4 pb-3">
        ...
        <input type="checkbox" name="categories" value="{{$category->id}}" @if (in_array($category->id, explode(",", $f_categories))) checked="checked" @endif>
        {{$category->name}}
        </span>
  ...
  @endif
    ...
    @endforeach
      ...
      {{$listProducts->withQueryString()->links('pagination::bootstrap-5')}}
  ...
  <form id="frmfilter" method="GET" action="{{ route('shop.index') }}">
    <input type="hidden" name="page" value="{{$listProducts->currentPage()}}">
...
@push('scripts')
  <script>
    ...
    $("input[name='brands']").on("change", function name(params) {
      var brands = "";

...
@section('content')
  ...
  <main class="pt-90">
    ...
    <div class="row">
      ...
      <div class="product-single__media" data-media-type="vertical-thumbnail">
        ...
        <div class="swiper-container">
          ...
          <div class="swiper-slide product-single__image-item">
            <img loading="lazy" class="h-auto" src="{{ asset('uploads/products') }}/{{$productBySlug->image}}"
            width="674" height="674" alt="" />
            <a data-fancybox="gallery" href="{{ asset('uploads/products') }}/{{$productBySlug->image}}"
          ...
          @foreach (explode(',', $productBySlug->images) as $gimg)
        <div class="swiper-slide product-single__image-item">
      ...
      @endforeach
        ...
        <div class="swiper-container">
          ...
          <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
            src="{{ asset('uploads/products/thumbnalis') }}/{{$productBySlug->image}}" width="104" height="104"
            alt="" /></div>

          @foreach (explode(',', $productBySlug->images) as $gimg)
        <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
        src="{{ asset('uploads/products/thumbnalis') }}/{{$gimg}}" width="104" height="104" alt="" /></div>
      @endforeach
      ...
      <h1 class="product-single__name">{{$productBySlug->name}}</h1>
      ...
      <div class="product-single__price">
        ...
        @if ($productBySlug->sale_price)
      <s>${{$productBySlug->regular_price}}</s> ${{$productBySlug->sale_price}}
    @else
    <span>${{$productBySlug->regular_price}}</span>
  @endif
        </span>
      </div>
      <div class="product-single__short-desc">
        <p>{{$productBySlug->short_description}}</p>
      </div>
      @if (Cart::instance('cart')->content()->where('id', $productBySlug->id)->count() > 0)
    ...
    @else
      ...
      <input type="hidden" name="id" value="{{$productBySlug->id}}">
      <input type="hidden" name="quantity" value="{{$productBySlug->quantity}}">
      <input type="hidden" name="name" value="{{$productBySlug->name}}">
      <input type="hidden" name="price"
        value="{{$productBySlug->sale_price == '' ? $productBySlug->regular_price : $productBySlug->sale_price}}">
    ...
    @endif
      <div class="product-single__addtolinks">
        ...
        @if(Cart::instance("wishlist")->content()->Where('id', $productBySlug->id)->count() > 0)
      <a href="javasript:void(0)" class="menu-link menu-link_us-s add-to-wishlist filled-heart"><svg width="16"
  ...
  @endif
      ...
      <div class="product-single__meta-info">
        <div class="meta-item">
        <label>SKU:</label>
        <span>{{$productBySlug->SKU}}</span>
        </div>
        <div class="meta-item">
        <label>Categories:</label>
        <span>{{$productBySlug->category->name}}</span>
        </div>
        <div class="meta-item">
        <label>Tags:</label>
        <span>NA</span>
